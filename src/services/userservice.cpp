...
void UserService::deleteUser(const QVariantMap &user)
{
    //    QString path = QString::fromLatin1("users/delete/");
    //    path.append(user[QString::fromLatin1("id")].toString());
    auto path = QString::fromLatin1("/users/delete/").append(user[QString::fromLatin1("id")].toString());
    this->url->setPath(path);
    this->request->setUrl(*url);
    qDebug() << request->url().toString();
    auto reply = this->netManager->deleteResource(*request);
